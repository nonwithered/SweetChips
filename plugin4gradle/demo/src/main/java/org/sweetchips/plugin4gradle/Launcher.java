...
public class Launcher {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        Debugger.init(properties);
        config();
        Debugger.launch(in, out, tmp, (i, o) ->
                new TransformInvocationImpl(i, o, Arrays.stream(dirs).map(Paths::get).collect(Collectors.toList())));
    }

    private static void config() {
        Debugger.apply(Util.NAME);
        Debugger.config("SweetChips", (UnionExtension it) -> {
            it.incremental(true);
            it.addPrepare(Collections.singletonMap("doLast", Collections.singletonList("org.sweetchips.visitors.UncheckcastPrepareClassVisitor")));
            it.addTransform(Collections.singletonMap("doLast", Arrays.asList("org.sweetchips.visitors.HideTransformClassNode", "org.sweetchips.visitors.UncheckcastTransformClassVisitor")));
        });
        Debugger.apply("ConstSweeper");
        Debugger.config("ConstSweeper", (ConstSweeperExtension it) -> {
            it.attach(null);
            ...
            while ((entry = zipInputStream.getNextEntry()) != null) {
                ...
                if (fileName.endsWith(".properties")) {
                    ...
                    Collections.list(property.propertyNames()).stream()
                            ...
                            .forEach(it -> map.put(fileName.substring(0, fileName.indexOf(".properties")), property.getProperty((String) it)));
                }
            }
        }
    }
